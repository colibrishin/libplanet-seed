    ...
    public class PreEvaluationBlockHeader : IPreEvaluationBlockHeader
    {
            ...
            HashDigest<SHA256> preEvaluationHash)
        {
            ...
            else if (metadata.LastCommit is { } commit)
            {
                ...
                {
                    var committedVotes = votes.Count(x => x.Flag == VoteFlag.Commit);

                    // TODO: The `votes.Length` must be replaced with actual known Validator count.
                    // This can be retrieved from `Libplanet.Net`. This seems to be an another
                    // bullet point for separating `Consensus` from `Libplanet`. Due to this, the
                    // validating consensus related data should be done lazy.

                    // Exceptional case: (Height 1, Round 0) The first consensus after genesis block
                    // cannot have last commit. The boundary value depends on migration block index
                    // value.
                    if (committedVotes <= votes.Length * 2 / 3 &&
                        commit.Height != 1 && commit.Round != 0)
                    {
                        throw new InvalidBlockLastCommitException(
                            $"The block #{metadata.Index}'s lastcommit's votes " +
                            $"does not meet the criteria of +2/3 committed vote. " +
                            $"More than {votes.Length * 2 / 3} votes should be presented, " +
                            $"however, contains {committedVotes} votes.");
                    }
